#!/usr/bin/env python3
#
# Membrane resistance exploration at lowered Ko
#
import os
import numpy as np
import matplotlib
...
dv = float(sys.argv[2])
dt = float(sys.argv[3])
conf = f'k={k}, dv={dv}, dt={dt}'

# Get path for figure
fpath = shared.figure_dir()
fname = 'figure-s11c-rm'
shared.splash(fname, conf)
...
# Show / store
path = f'{fname}'
path += f'-ko-{shared.fstr(k)}'
path += f'-dv-{shared.fstr(dv)}'
path += f'-dt-{shared.fstr(dt)}'
