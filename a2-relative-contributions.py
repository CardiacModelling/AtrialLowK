#!/usr/bin/env python3
#
# Supplementary figure: Relative contributions of the major ionic currents in
# all models.
#
import os
import sys
...
fname = 'figure-a2-relative-contributions'
...
ax.set_title('Modified Grandi')
ax.set_xlabel('Time (s)')
ax.set_yticklabels([])
ax.set_xlim(0, tmax)
ax.set_ylim(-1.02, 1.02)
mp.cumulative_current(d, currents, ax, colors=colours, normalise=True)
text(ax, 0.950, 0.840, 'INaK')
text(ax, 0.950, 0.595, 'IK1')
text(ax, 0.950, 0.400, 'INaCa')
text(ax, 0.950, 0.245, 'ICa,B')
text(ax, 0.950, 0.070, 'INa,B')
text(ax, 0.217, 0.542, 'IKur')
text(ax, 0.225, 0.445, 'ICaL')
text(ax, 0.300, 0.900, 'Ito')
ax.arrow(0.202, 0.900, -0.085, -0.090, transform=ax.transAxes, zorder=999)
text(ax, 0.690, 0.545, 'IClB')
ax.arrow(0.700, 0.545, 0.086, -0.055, transform=ax.transAxes, zorder=999)
'''
