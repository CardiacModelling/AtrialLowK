#!/usr/bin/env python3
#
# Instantaneous IV curves at low potassium
#
import os
import sys
import matplotlib
...
grid = matplotlib.gridspec.GridSpec(2, 3, wspace=0.45, hspace=0.35)
...
for voltage, current, iv_current, label in zip(vts, its, iv_currents, labels):
    ax2.plot(iv_voltages, iv_current, label=label)
    ax2.plot(voltage, current, 'ks')
plt.gca().set_prop_cycle(None)
vts, its, iv_voltages, iv_currents = shared.quasi_instantaneous_iv(
